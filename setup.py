...
# main sdeconv __init__ can detect if it is being loaded by the setup
# routine, to avoid attempting to load components that aren't built yet:
# the numpy distutils extensions that are used by sdeconv to
# recursively build the compiled extensions in sub-packages is based on the
# Python import machinery.
builtins.__sdeconv_SETUP__ = True
...
URL = 'https://sylvainprigent.github.io/sdeconv'
DOWNLOAD_URL = 'https://github.com/sylvainprigent/sdeconv'
...
# We can actually import a restricted version of sdeconv that
# does not need the compiled code
import sdeconv
import sdeconv._build_utils.min_dependencies as min_deps  # noqa


VERSION = sdeconv.__version__
...
class CleanCommand(Clean):
    ...
    def run(self):
        ...
        if os.path.exists('build'):
            shutil.rmtree('build')
        for dirpath, dirnames, filenames in os.walk('sdeconv'):
...
try:
    ...
    class build_ext_subclass(build_ext):
        def build_extensions(self):
            from sdeconv._build_utils.openmp_helpers import get_openmp_flag

            if sdeconv._OPENMP_SUPPORTED:
...
# To automate release of binary packages for sdeconv we need a tool
...
def configuration(parent_package='', top_path=None):
    ...
    from sdeconv._build_utils import _check_cython_version
...
def check_package_status(package, min_version):
    ...
    req_str = "sdeconv requires {} >= {}.\n".format(
        package, min_version)

    instructions = ("Installation instructions are available on the "
                    "sdeconv website: "
                    "http://sdeconv.org/stable/install.html\n")
...
def setup_package():
    ...
    if len(sys.argv) == 1 or (
        ...
        # pip is used to install sdeconv when Numpy is not yet present in
        # the system.
